...
const README_TMPL: &str = include_str!("../templates/README.md");
const LICENSE_TMPL: &str = include_str!("../templates/LICENSE");
const GITIGNORE_TMPL: &str = include_str!("../templates/.gitignore");
...
fn git_init() -> Result<(), Box<dyn Error>> {
    ...
    fs::write(path.join("README.md"), README_TMPL)?;
    fs::write(path.join("LICENSE"), LICENSE_TMPL)?;
    fs::write(path.join(".gitignore"), GITIGNORE_TMPL)?;
    ...
    run_cmd(
        "gh",
        &["repo", "create", &repo_name, visibility, "--confirm"],
    )?;
	run_cmd("git", &["init"])?;
	run_cmd(
        "git",
